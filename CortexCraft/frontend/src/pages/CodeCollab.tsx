...
function Lobby({ initialRoom, onJoin }: { initialRoom?: string; onJoin: (room: string, name: string) => void }) {
  ...
  const [nameInput, setNameIn]  = useState(() => {
    try { return JSON.parse(localStorage.getItem('cortexcraft_user') || '{}').name || ''; } catch { return ''; }
  });
...
export default function CodeCollab() {
  ...
  const handleLobbyJoin = (room: string, name: string) => {
    ...
    try {
      const stored = JSON.parse(localStorage.getItem('cortexcraft_user') || '{}');
      if (stored.name === name) user = stored;
    } catch { /**/ }
    localStorage.setItem('cortexcraft_user', JSON.stringify(user));
    setCurrentUser(user);
    setActiveRoom(room);
    setHasJoined(true);
